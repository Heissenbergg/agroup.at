...
<body>
	...
	<div id="working_area_header">
		<h4>KATEGORIJE PROIZVODA / <?php echo mb_strtoupper(category_name('our_products', Input::get('id'))); ?></h4>
	</div>
	<div id="working_area">
			...
			while($object = $dbQuery -> fetch()){
				if($object['cat_id'] == $category_id){
					...
					<div class="form_output_preview" title="Pogledajte sadržaj kategorije">
						<a target="_blank" href="../our_product.php?id=<?php echo $object['id']; ?>"><p>Pogledajte kao korisnik</p></a>
					</div>
					<div class="form_output_delete form_output_delete_single" title="Uredite artikal">
						<a href="edit_products.php?id=<?php echo $object['id']; ?>"><i class="fa fa-edit"></i></a>
					</div>
					<div class="form_output_delete" title="Obrište kategoriju" onclick="deleteObject('all_products' , '<?php echo $object['id']; ?>');">
		...
		<div class="form_output form_output_add_ form_output_add_new">
			<a href="insert_products.php?cat_id=<?php echo $category_id; ?>">UNESITE PROIZVOD</a>
		</div>
	</div>
</body>
</html>
